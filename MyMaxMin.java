...
{	
	...
	public static class MaxTemperatureMapper extends Mapper<LongWritable,Text,Text,Text> 
	{
		...
		public void map(LongWritable arg0,Text Value,Context context) throws IOException,InterruptedException 
		{
			...
			{	
				...
				if(precip<2.5)
					context.write(new Text("Light Rain "+date),new Text(String.valueOf(precip)));
				if(precip>2.5 && precip<7.6)
					context.write(new Text("Moderate Rain "+date),new Text(String.valueOf(precip)));
				if(precip>7.6)
					context.write(new Text("Heavy Rain "+date),new Text(String.valueOf(precip)));
			}
		}
	}
	//Reducer
	...
	{
		Job job=Job.getInstance(new Configuration(),"weather prediction");
